...
temp_contours, _ = cv2.findContours(car_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
...
for frame_i in range(10000):
    ...
    observation, reward, done, info = env.step(np.array([0, 0.01, 0]))
    ...
    contours_track, _ = cv2.findContours(mask_track_copy, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
